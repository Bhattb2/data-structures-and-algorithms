...
const count = (target, input) => {
  ...
  for (var i = 0; i < input.length; i ++) {
      input [i].map(num => {
          target === num ? counter ++: counter = counter + 0;
      })
  }
  return counter;
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(subarray => subarray.filter(element => typeof element === 'number' && element % 5 === 0).map(element => Math.pow(2, element)));
