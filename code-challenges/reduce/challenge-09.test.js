...
const countNumberOfElements = (arr) => {
  // Solution code here...
  const sum = arr.reduce((sumThusFar, value, index) =>{
      return sumThusFar = sumThusFar + 1;
  }, 0);
  return sum;
};
...
const returnNames = (arr) => {
  // Solution code here...
  return arr.reduce((name, obj, idx) => {name.push(obj.name);
      return name;
...
const reversedString = (str) => {
  ...
  let reversename = splitarr.reduce((newstr, chr, idx) => {
      return chr + newstr;
  }, '');
  return reversename;
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  let children = arr.reduce((answer, value) => {
      if (value.children){
          answer += value.children.length;
        }
        return answer;
  },0);
  return children;
...
const calculateAverage = (arr) => {
  ...
  const newNum = arr.reduce ((accl, num, idx) => {
      sum = sum + num;
      count = count + 1;
      accl.sum = sum;
      accl.count = count;
      ...
      let avg = newNum.sum/newNum.count;
      return avg;
...
const countPrimeNumbers = (arr) => {
  // Solution code here...
  let count = arr.reduce((counter, value) => {
      if (isPrime(value)) {
          counter = counter + 1;
      }
      return counter;
